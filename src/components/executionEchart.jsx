...
export default class ExecutionEchart extends React.PureComponent {
  ...
  setEcharts = () => {
    ...
    const executionChart = echarts.init(
      document.getElementById(`execution-echart-${index}`)
    );
    // 绘制图表
  ...
  render() {
    const { className, index, name, ...other } = this.props;
    ...
    return (
      <div className={cls}>
        ...
        <div
          id={`execution-echart-${index}`}
          className={`${prefixCls}-echart`}
        />
      </div>
    );
  }
